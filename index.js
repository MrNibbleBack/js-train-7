...
function createSymbolForKey(key) {
  ...
  const sym = Symbol.for(key.toString());
  return sym;
}
...
function compareSymbolsForKey(symbol1, symbol2) {
  ...
  const sym1 = createSymbolForKey(symbol1);
  const sym2 = createSymbolForKey(symbol2);
  return sym1 === sym2;
}
...
let myObject = {
  ...
  next() {
    return this.currentIndex < this.category.length
      ? // ? { done: false, value: this.category[this.currentIndex++] }
